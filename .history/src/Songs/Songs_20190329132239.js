...
class Songs extends Component {
  ...
  mouseOver = e => {
    this.changeIconsButton(1);
    this.reference1.current.classList.remove("fa-music");
    this.reference1.current.classList.add("fa-play");
  };

  mouseOut = e => {
    this.reference1.current.classList.remove("fa-play");
    this.reference1.current.classList.add("fa-music");
  };
  ...
  createRowsTable = (song, key) => {
    ...
    return (
      <tr key={key}>
        <td>
          <button
            onMouseOut={key => this.mouseOut(key)}
            onMouseOver={key => this.mouseOver(key)}
            type="button"
