...
class Songs extends Component {
  ...
  changeIconsButton = (key, removeItem, addItems) => {
    let reference = `${'reference'+items}`;
    this.['reference'+items].current.classList.remove(removeItem);
    this.reference.current.classList.add(addItems);
  };
  mouseOver = items => {
    this.changeIconsButton(items, "fa-music", "fa-play");
  };

  mouseOut = items => {
    this.changeIconsButton(items, "fa-play", "fa-music");
  };
