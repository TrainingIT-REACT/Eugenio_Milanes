...
class Songs extends Component {
  ...
  mouseOut = id => e => {
    console.log(id);
    this[`icons${id}`].className("fa fa-music");
  };

  mouseOver = id => e => {
    console.log(id);
    this[`icons${id}`].className("fa fa-play");
  };
  ...
  createRowsTable = (song, key) => {
    ...
    return (
      <tr key={key}>
        <td>
          <button
            onMouseOut={() => this.mouseOut(key)}
            onMouseOver={() => this.mouseOver(key)}
            type="button"
